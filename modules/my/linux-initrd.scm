...
the derivations referenced by EXP are automatically copied to the initrd."
  ...
  (define builder
    ...
    (with-imported-modules (source-module-closure
	  ...
	  ;;(module-set! (resolve-module '(gnu system mapped-devices)) 'open-luks-device (module-ref (resolve-module '(my linux-initrd)) 'my-open-luks-device))
          (mkdir #$output)
    ...
    (map (lambda (md)
	   ...
	   ;;this will not work if there is a non-luks device :(
           (let* ((source  (mapped-device-source md))
                  ...
                  (type    (mapped-device-type md))
                  (open    (mapped-device-kind-open type)))
             ((module-ref (resolve-module '(my linux-initrd)) 'my-open-luks-device) source targets)))
         mapped-devices))
